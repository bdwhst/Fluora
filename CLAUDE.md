...
FluoraMini's integrator is the portable NEE + MIS path tracer (`src/mini/pathtrace_gpu.h` over `src/core/light_shared.h`, `bsdf_shared.h`, `spectrum_shared.h`): uniform light selection over emissive objects/triangles plus the env map, power-heuristic MIS, one shared `miniShadeVertex` for both modes, and a shadow-ray queue stage in wavefront mode. Homogeneous participating media (`src/core/medium_shared.h`) are delta-tracked at the hero wavelength inside `miniTrace`, which also passes rays through medium-interface surfaces; real scatters become Henyey-Greenstein vertices (a fourth `wf_shade` specialization, `MINI_MAT_MEDIUM`), shadow rays carry spectral contributions and apply analytic transmittance, and paths carry the PBRT-v4 spectral-MIS ratio `r` (exactly 1 without media, so media-free renders are unchanged). Per the M4 part-2 decision in `docs/metal-rhi-design.md`, remaining renderer features are ported into this core rather than the old renderer being migrated.
