...
**Materials use a newer, preferred pattern** (`taggedindex.h`): persistent storage is a 32-bit `MaterialHandle` ({tag, index}) plus type-segregated pools (`MaterialPool` in `Scene`, uploaded to a POD `MaterialPoolView` in `SceneInfoDev`); kernels resolve handles to a transient `MaterialPtr` via `pool.resolve<MaterialPtr>(handle)`. This exists so device-visible data contains no raw host pointers (a prerequisite for a future Metal backend — see the RHI sketch in `src/rhi/`, which is documentation-grade and not yet in the build). When touching lights/media/spectra storage, prefer migrating them to this handle pattern over adding new `alloc.new_object` pointer graphs.
