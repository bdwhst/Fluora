...
./build-mac/bin/FurnaceTest     # white-furnace energy checks: diffuse sphere + scattering-only fog box (drives FluoraMini; catches both-mode integrator bugs)
```
...
FluoraMini's integrator is the portable NEE + MIS path tracer (`src/mini/pathtrace_gpu.h` over `src/core/light_shared.h`, `bsdf_shared.h`, `spectrum_shared.h`): uniform light selection over emissive objects/triangles plus the env map, power-heuristic MIS, one shared `miniShadeVertex` for both modes, and a shadow-ray queue stage in wavefront mode. Participating media (`src/core/medium_shared.h`) are delta-tracked at the hero wavelength inside `miniTrace` (PBRT-v4 `SampleT_maj`: a majorant-segment iterator, one segment for a homogeneous medium or a brick DDA for a grid, with null collisions and emission at every collision), which also passes rays through medium-interface surfaces; real scatters become Henyey-Greenstein vertices (a fourth `wf_shade` specialization, `MINI_MAT_MEDIUM`), shadow rays carry spectral contributions and apply transmittance (analytic for homogeneous media, ratio tracking on a forked RNG stream for grids), and paths carry the PBRT-v4 spectral-MIS ratio `r` (exactly 1 without media, so media-free renders are unchanged). Per the M4 part-2 decision in `docs/metal-rhi-design.md`, remaining renderer features are ported into this core rather than the old renderer being migrated.
...
- `.json` — newer format with richer material descriptions (spectral eta/k, named spectra from `SpectrumConsts/`), volumes, and proper camera DOF parameters. Loader: `Scene::loadJSON` in `scene.cpp`; `loadJsonScene` in the core (PLY/OBJ/glTF/GLB/inline meshes — model paths dispatch on the real extension, and glTF files supply their own materials when the object has no MATERIAL (base color + texture, uniform-metallic conductors, KHR transmission/ior glass; per-texel metallic masks and normal maps are not honored) — EXR textures, env SCALE/MAXRGB, homogeneous and NanoVDB media with medium interfaces, blackbody emission from a temperature grid (LESCALE/TEMPSCALE/TEMPOFFSET), the camera MEDIUM, and thin-lens DOF via LENS_RADIUS/FOCAL_LEN — 0 radius or missing FOCAL_LEN means pinhole, and pinhole scenes draw no lens samples so their renders stay bitwise unchanged). NanoVDB files are read on the host by `src/core/volume_loader.cpp` and re-bricked into flat buffers (`core/medium_shared.h`), so the device never sees NanoVDB; the scene `.nvdb` files are ZIP-compressed, hence zlib. Interface-only objects are surfaceless boundaries: camera paths and shadow rays pass through and switch medium by the PBRT rule (leaving along the geometric normal enters OUTSIDE, so a closed outward-wound box with INSIDE set to a medium contains it). Prefer this for new scenes. Paths inside both formats are joined with the scene's directory, so the `../scenes/...` form works from the repo root or `build/`.
